...
string VisitInterp::visit(IdentNode* id, Memory* mem, int flag) {
    std::ostringstream oss;
    int value = Interpreter::val(id->getChildren()[0]->accept(this, mem), mem->getStack());
    oss << value;
    return oss.str();
}
...
string VisitInterp::visit(IncrementNode* in, Memory* mem, int flag) {
    ...
    int identifier = atoi(in->getChildren()[0]->accept(this, mem).c_str());
    identifier++;
    /*  std::ostringstream oss;
 oss << identifier;
    ...
    Interpreter::affecterVal(identifier, val(value,mem), mem->getStack());*/
...
string VisitInterp::visit(OrNode* num, Memory* mem, int flag) {
    ...
    //return (leftChild || rightChild ) // should return a string
    if(leftChild || rightChild ) return "true";
    else return "false";
